...
function deleteBtnEvent(event) {
  ...
  const confirmToDelete = confirm(`Are you sure you want to delete ${songName.textContent}?`);
  if(confirmToDelete) {
...
function addNewSong() {
  ...
  addButton.addEventListener('click', function() {
    ...
    const songMatchedArr = [...songNamesList].filter((item) => item.textContent === newSong.name);

    if(!newSong.name || newSong.name.length <= 3) {
      errorText.textContent = 'Song name should not be empty or shorter than 3 characters!';
